...
st.markdown("""
    <style>
        ...
        .input-container { background: rgba(255, 255, 255, 0.6); border-radius: 20px; padding: 2rem 3rem; margin: 2rem auto; max-width: 1000px; box-shadow: 0 8px 32px rgba(31,38,135,0.37); backdrop-filter: blur(10px); border: 1px solid rgba(255,255,255,0.18); }
        .stButton>button { background: linear-gradient(90deg, #4776E6 0%, #8E54E9 100%); color: white; border-radius: 50px; padding: 12px 30px; font-size: 18px; font-weight: 700; border: none; transition: all 0.3s ease-in-out; box-shadow: 0 4px 15px rgba(71,118,230,0.75); display: block; margin: 1.5rem auto 0 auto; }
        .stButton>button:hover { transform: translateY(-3px); box-shadow: 0 8px 25px rgba(142,84,233,0.6); }
...
if model is None or model_columns is None:
    st.error("🔴 Critical Error: Model files ('model.joblib', 'model_columns.joblib') not found or incompatible. Please verify they are uploaded and compatible.")
else:
    # --- Input Section ---
    ...
    with col8:
        street = st.selectbox("Street Address", options=STREETS_IN_MODEL, help="Street address is for reference and not used in this model.")
    ...
    if predict_button:
        try:
            ...
            with col_img:
                st.image("https://cdn-icons-png.flaticon.com/512/4813/4813274.png", width=150)
            with col_price:
